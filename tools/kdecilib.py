...
class BuildManager(object):
	...
	def perform_rsync(self, source, destination):
		...
		print("Rsyncing from %s to %s" % (source, destination))
	...
	def sync_dependencies(self):
		...
		for candidate, candidateBranch in self.dependencies:
			...
			# Do some OSX specific stuff
			# We need to evaluate if this is even the way we want to do things...
			if sys.platform == "darwin":
				HOME=expanduser("~")
	...
	def execute_tests(self):
		...
		# Special Case: Marble's build system doesn't use ECM, but is able to use stuff that does use ECM
		# This causes problems with ASAN, so we force inject ASAN into all processes running in a Marble build
		# It's a terrible hack, but the Marble developers refuse to depend on ECM so we have no other choice (other than not running the Marble tests)
		if self.project.identifier == 'marble' and os.path.exists('/usr/lib/x86_64-linux-gnu/libasan.so.2.0.0'):
			# Inject ASAN for Marble....
			runtimeEnv['LD_PRELOAD'] = '/usr/lib/x86_64-linux-gnu/libasan.so.2.0.0'
...
def load_projects( projectFile, projectFileUrl, configDirectory, moduleStructure ):
	...
	if not os.path.exists(projectFile) or time.time() > os.path.getmtime(projectFile) + 60*60:
		...
		# Download the projects list
		urllib.urlretrieve(projectFileUrl, tempFilename)
		# Replace our existing file
		shutil.move(tempFilename, projectFile)
